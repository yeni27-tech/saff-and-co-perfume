...
(function (window) {
  ...
  HorizontalNav.prototype = {
    ...
    showPage: function (item) {
      ...
      (document.querySelectorAll("." + self.options.page.show) || []).forEach(
        (n) => n.classList.remove(self.options.page.show)
      );
      (document.querySelectorAll("." + self.options.page.nav) || []).forEach(
        (n) => n.classList.remove(self.options.page.nav)
      );
